...
module.exports = (apiKey, deposits, withdrawals) => {
  ...
  Object.keys(config.get('testData.currencies')).forEach(currency => {
    const n1 = deposits.filter(deposit => deposit.user === apiKey && deposit.currency === currency)
    const n2 = withdrawals.filter(withdrawal => withdrawal.user === apiKey && withdrawal.currency === currency)
