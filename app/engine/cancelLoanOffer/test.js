...
test.serial(t => {
  ...
  engine.createLoanOffer('me', 'BTC', 100, 90, 0, 1, 86400, 5000)
  engine.createLoanOffer('others', 'BTC', 200, 70, 0, 1, 86400, 6000)
  ...
  t.is(engine.loanOffers.filter(offer => offer.open).length, 2)
  t.is(engine.loanOffers.filter(offer => offer.open && offer.loanID === 6000).length, 1)
  ...
  // 4.1 Should only be one open order, and none for loanID 5000
  t.is(engine.loanOffers.filter(offer => offer.open).length, 1)
  t.is(engine.loanOffers.filter(offer => offer.open && offer.loanID === 5000).length, 0)
  ...
  // 5.1 Should only be one open order
  t.is(engine.loanOffers.filter(offer => offer.open).length, 1)
})
