...
test(t => {
  ...
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.TOTAL_MUST_BE_AT_LEAST_0_0001}
  t.deepEqual(actual, expected)

  // 2. Rate
  mockRequestBody = {rate: 'a'}
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.INVALID_RATE_PARAMETER}
  t.deepEqual(actual, expected)
  ...
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.INVALID_AMOUNT_PARAMETER}
  t.deepEqual(actual, expected)
  ...
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.REQUIRED_PARAMETER_MISSING}
  t.deepEqual(actual, expected)

  // 6. currencyPair is not in the market
  mockRequestBody = {currencyPair: 'catfood', amount: 1, rate: 0.0001}
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.INVALID_CURRENCY_PAIR_PARAMETER}
  t.deepEqual(actual, expected)
  ...
  actual = sell('key', mockRequestBody, engine)
  // expected = {error: c.sell.INVALID_CURRENCY_PAIR_PARAMETER} sb ok
  // t.deepEqual(actual, expected)
  ...
  actual = sell('key', mockRequestBody, engine)
  expected = {error: c.sell.NO_MORE_THAN_ONE}
  t.deepEqual(actual, expected)
