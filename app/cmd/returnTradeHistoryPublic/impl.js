...
module.exports = (reqQuery, engine) => {
  ...
  if (Object.keys(reqQuery).length === 0) return {'error': c.PLEASE_SPECIFY_A_CURRENCY_PAIR}

  // The currencyPair must be in the list of valid markets
  if (config.get('testData.markets').indexOf(reqQuery.currencyPair) === -1) return {'error': c.INVALID_CURRENCY_PAIR}
